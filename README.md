...
с инструментами MCP: цикл «поиск → проверка статуса → дочитывание → связи» ведёт LLM без режима
размышлений (бюджет 8 вызовов, FR-1), итоговый ответ с самопроверкой и ссылками на фрагменты пишет
LLM с размышлениями (N9). В консоли видны шаги агента, заметки после поиска, стрим ответа и время.
